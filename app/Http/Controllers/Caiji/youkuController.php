...
class youkuController extends Controller
{
	...
	//优酷采集视频
	public function youku($url="http://cj.tv6.com/mox/inc/youku.php")
	{	
		...
		if ($type == "film") {//如果是电影的话，要审核
			...
			if ($id != "404") {
				foreach ($id as $key => $value) {
					...
					// var_dump($data);
					$get_playfrom = DB::table('vods')->where('id', '=', $value->id)->first();
					$all_playfrom = explode('$$$', $get_playfrom->playfrom);
					$all_dd = explode('$$$', $get_playfrom->dd);
					...
					if (count($all_data) == 1) {
						...
						$data = "";
						for ($i=0; $i < count($all_dd); $i++) { 
							$dd = $all_dd[$i];
							preg_match('/http.*?com/', $dd, $get_url);
							foreach ($all_data as $key => $value) {
								preg_match('/http.*?com/', $value, $get_url1);
								if ($get_url == $get_url1) {
									$data .= $value."$$$";
								}
							}
	...
	private function continu_judge($alldata){
		foreach ($alldata as $key => $value) {
			...
			if ($resource_data) {
				$video_data['all_dd_resource'] = trim($resource_data['downurl'], '$$$');
			...
			$select_db = DB::table('vods')->where('name', 'like', '%'.$video_data['name'].'%')->first();
			if (empty($select_db)) {
	...
	public function auto_collection($collection)
	{
		...
		if ($type_name_find_all_data) {
			...
			foreach ($type_name_find_all_data as $key => $value) {
				$alldata['downurl'] = $alldata['downurl'].'$$$'.$value['downurl'];
