...
class TtffController extends Controller
{
	// 采集电视剧的地址
    ...
    //采集动漫的地址
    protected $comic_url = 'http://dongman.2345.com';
	//这是代理
    ...
    {
        //将页数加入到cache
        Cache::put('page', $this->page, 32);
        //将详情页的个数加入到cache
        Cache::put('num', $this->num, 32);
    }
    ...
    //电影自动采集的入口
    public function film_auto_apis(Request $request)
    {
        //定义类型
        $admin = array();
        $admin['type_name'] = '电影'; 
    ...
    //动漫自动采集的入口
    public function comic_auto_apis(Request $request, $type='upd')
    {
        //定义类型
        $admin = array();
        $admin['type_name'] = '动漫';
        ...
        if ($type == 'upd') {//这里采集更新的视频站
            //设置开启守护进程
           $start_beanbun = $this->open_beanbun(isset($request->url)?$request->url:$this->comic_url."/ltztlianzai/$$$/", $admin, 'collection_2345video');
        } else if ($type == "all") {//这里采集所有更新的动漫
            //设置开启守护进程
            $start_beanbun = $this->open_beanbun(isset($request->url)?$request->url:$this->comic_url.'/lt/$$$/', $admin, 'collection_2345video');//采集所有动漫
        } else {
            $ip = $this->get_ip();
            return '非法了，嘤嘤嘤！！！';
        }
    ...
    private function _start_collection($url="", $admin)
    {
        ...
        //获取文件(缓存)里面是否存在这个页数，有的话就page开始，没有的话就从第一1开始
        $page = trim(file_get_contents('page.txt'));//文件
        // $page = Cache::get('page');//缓存
        if ($page) {
            //判断是否等于最大页数
            if ($page >= $maxpage) {
                // 是的话，删除
                // Cache::forget('page');
                file_put_contents('page.txt', '');
            }
        }

        //重新获取是否存在这个页数
        $page = trim(file_get_contents('page.txt'));
        // $page = Cache::get('page');
        $page = $page?$page:1;
    	// 循环采集数据
    	for ($i=$page; $i <= intval($maxpage); $i++) {
            file_put_contents('page.txt', $i);sleep(0.1);//加入时间要有休眠，否则加入失败（这里设置0.1秒）
    		// 循环采集每个页面
            //判断采集类型
            switch (parse_url($url)['host']) {
                case 'tv.2345.com':
            ...
            if ($arr_data) {
                ...
                preg_match_all('/<div class=[\'|\"]pic[\'|\"].*?<\/div>/ism', $arr_data[0], $li_date);
                $li_date = $li_date[0];
                //循环获取到每个详情页地址
                for ($j=0; $j < count($li_date); $j++) {
                    preg_match('/<em>(.+?)<\/em>/', $li_date[$j], $em);
                    if (isset($em[1])) {
                        preg_match('/[-.0-9]{1,}/', $em[1], $em);
                        $admin['note'] = $em[0];
                    } else {
                        $admin['note'] = 1;
                    }
    ...
    protected function get_detail_data($url = "", $admin) {
    	...
    	//获取到详情页的地址后爬取网站数据
        // $content = $this->try_to_collect('http://dianying.2345.com/detail/196183.html');
